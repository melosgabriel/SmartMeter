...
esp_err_t i2c_master_write_slave(uint8_t *data, size_t size)
{
	...
	i2c_master_write_byte(cmd, (SLAVE_7B_ADDRESS << 1) | WRITE_BIT,
			      ACK_CHECK_EN);
	...
	esp_err_t ret = i2c_master_cmd_begin(I2C_PORT, cmd,
					     100 / portTICK_RATE_MS); // I changed to 100 ms
...
esp_err_t i2c_master_read_slave(uint8_t *data, size_t size)
{
	...
	i2c_master_write_byte(cmd, SLAVE_7B_ADDRESS << 1 | READ_BIT,
			      ACK_CHECK_EN);
	if (size > 1)
...
esp_err_t ade_write_reg_8(ade_reg_t addr, uint8_t data)
{
	...
	return ade_write_reg(addr, data8, num_bytes);
}
...
esp_err_t ade_read_reg(ade_reg_t addr, uint8_t *data, size_t size)
{
	...
	/** SECOND PART */
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (SLAVE_7B_ADDRESS << 1) |
...
int ade_read_reg_8(ade_reg_t addr)
{
	...
	return uint8toint(data, num_bytes);
}
...
esp_err_t ade_init(void)
{
	...
	ade_write_reg_16(ADE_COMPMODE, 0x41FF);
#endif
